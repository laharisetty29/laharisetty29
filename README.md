#  Lahari Gadamsetty

Welcome to my GitHub profile!  
I am a motivated **Computer Science & Engineering (Data Science)** student with a strong interest in  
**Full Stack Development, Data Analysis, and Backend Systems**.  
I enjoy building practical applications, solving real-world problems, and continuously improving my technical skills.

---

##  About Me

**Education**  
- **B.Tech – Computer Science & Engineering (Data Science)**  
  Srinivasa Ramanujan Institute of Technology (SRIT), Andhra Pradesh  
  Graduation Year: **2026** | CGPA: **7.6**

**Location**:Andhra Pradesh, India

**Career Interests**  
- Full Stack Web Development  
- Data Analysis & Visualization  
- Backend & Enterprise Application Development  

---

##  Technical Skills
- **Languages**: Python, Java, SQL, JavaScript, HTML, CSS  
- **Frameworks & Tools**: Django, React (basic knowledge), ABAP CDS  
- **Databases**: MongoDB, SQLite  
- **Development Tools**: Git, GitHub, VS Code, Microsoft Excel  
---

##  Projects

###  Flavor Fiesta – Full Stack Recipe Web Application
- Developed a recipe discovery and food ordering platform  
- Implemented OTP-based authentication and responsive UI  
- Tech Stack: **Flask, React, SQLite**
---

###  Ticket Booking System
- Designed a seat-based ticket booking system with snack ordering  
- Supports seat selection, order history, and seat-wise snack delivery  
- Tech Stack: **Flask, SQLite**
---

###  DocSpot – Seamless Appointment Booking for Healthcare
- Developed a healthcare appointment booking platform  
- Enables smooth interaction between patients and doctors  
- Tech Stack: **MERN Stack**  
---
For a comprehensive list, visit [My GitHub Repositories](https://github.com/laharisetty29?tab=repositories).
##  Certifications
- SAP Certified Associate – **Back-End Developer (ABAP Cloud)**  
- Full Stack Web Development (MERN) – SmartBridge  
- Python Programming – HackerRank  
- Wadhwani 21st Century Employability Skills (Intermediate)  

---

##  Contact Me

- 📧 Email: laharigadamsetty@gmail.com  
- 💻 GitHub: https://github.com/laharisetty29  
- 💼 LinkedIn: https://www.linkedin.com/in/laharigadamsetty  

---

 *Always eager to learn, build, and contribute to meaningful software solutions.*
